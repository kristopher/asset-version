...
Has the Rails asset versioning let you down? AssetVersion ensures your users NEVER load out of date assets. Cached assets
include this version in the file name so a version change guarantees a new
...
ASSET_VERSION constant in your environment.rb or an initializer like so:
...
option, it will automatically include the version in the filename.
  ...
  javacript_include_tag('application', 'extra', :cache => 'all')
...
License:: Copyright (c) 2008 Kristopher Chambers <mailto:kristopher.chambers@gmail.com>
